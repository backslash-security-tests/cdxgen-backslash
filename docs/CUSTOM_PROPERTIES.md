...
| `cdx:gem:*`                                                                                | RubyGems/Bundler                                                                    | Gem source/revision/tag/branch, ruby and rubygems constraints, executable and native extension presence, MFA requirement, prerelease/yanked status. The native platform is a purl qualifier, not a property  | Detect mutable VCS-sourced gems, install-time native compilation, and yanked/prerelease risk in resolved dependency sets                                                                                                       | [Package manager and language ecosystems](#inventory-packages)       | [3](#example-3)                  |
...
| `cdx:plugin:*`                                                                             | Claude Code plugin manifests and marketplace listings                               | Plugin name/version/author, and counts of contributed commands, subagents, skills, lifecycle hooks, and wired MCP servers                                                                                    | Inventory installable agent plugins as a supply-chain surface and flag plugins that register hooks or MCP servers for review                                                                                                   | [Cross-cutting BOM/service/build metadata](#inventory-cross-cutting) | [5](#example-5)                  |
| `cdx:registry:server:*`                                                                    | MCP Registry `server.json` manifests                                                | Server name/version, declared distributable package registry types and counts, package transport, and remote endpoint counts                                                                                | Inventory MCP servers published to the official registry, resolve their distributables to purls, and separate registry-hosted remotes from local packages                                                                      | [Cross-cutting BOM/service/build metadata](#inventory-cross-cutting) | [5](#example-5)                  |
| `cdx:agentic:*`                                                                            | Installed agentic CLI tools discovered on the host (`-t agentic`)                   | Tool key, home and support directories, tool version, and redacted structural counts of agents, MCP servers, tools, plugins, providers, skills, commands, and knowledge bases                                | Inventory which agentic coding CLIs (Kiro CLI, zcode, opencode, Claude Code, and similar) are installed on a developer or build host and how they are configured, without reading credentials or transcripts                    | [Cross-cutting BOM/service/build metadata](#inventory-cross-cutting) | [5](#example-5)                  |
| `cdx:mcpb:*`                                                                               | MCPB / DXT desktop-extension bundles                                                | Bundle manifest version, server type, tool/prompt counts, declared user-config field counts, secret-typed config indicators, and precompiled platform binary counts                                          | Inventory one-click installable local MCP servers as a supply-chain surface and flag bundles that inject secret configuration as environment variables or ship precompiled binaries                                            | [Cross-cutting BOM/service/build metadata](#inventory-cross-cutting) | [5](#example-5)                  |
| `cdx:safetensors:*`                                                                        | Safetensors model artifact headers                                                  | Tensor count and distinct tensor dtypes read from the safetensors header without loading tensor payloads                                                                                                     | Inventory local safetensors checkpoints and summarize their tensor shape without deserializing weights                                                                                                                        | [Cross-cutting BOM/service/build metadata](#inventory-cross-cutting) | [5](#example-5)                  |
...
| `cdx:skill:specValid`                            | Boolean cue that a SKILL.md satisfies the open Agent Skills frontmatter constraints                 | Warning / triage |
| `cdx:skill:hasScripts`                           | Marks a skill bundling executable `scripts/` the agent may run                                       | Warning / triage |
| `cdx:skill:hasReferences`                        | Marks a skill bundling on-demand `references/` documentation                                        | Informational    |
| `cdx:skill:hasAssets`                            | Marks a skill bundling `assets/` such as templates or data files                                    | Informational    |
| `cdx:plugin:description`                         | Describes a discovered Claude Code plugin or marketplace listing                                    | Informational    |
| `cdx:plugin:author`                              | Records the declared author of a Claude Code plugin                                                 | Informational    |
| `cdx:plugin:commandCount`                        | Counts slash commands a plugin contributes                                                          | Warning / triage |
| `cdx:plugin:agentCount`                          | Counts subagents a plugin contributes                                                               | Warning / triage |
| `cdx:plugin:skillCount`                          | Counts skills a plugin contributes                                                                  | Warning / triage |
| `cdx:plugin:hookCount`                           | Counts lifecycle hooks a plugin registers                                                           | Hard deny        |
| `cdx:plugin:mcpServerCount`                      | Counts MCP servers a plugin wires in                                                                | Hard deny        |
| `cdx:plugin:source`                              | Records the source reference of a marketplace-listed plugin                                         | Warning / triage |
| `cdx:registry:server:name`                       | MCP Registry server name declared in a `server.json` manifest                                       | Informational    |
| `cdx:registry:server:version`                    | MCP Registry server version from a `server.json` manifest                                           | Informational    |
| `cdx:registry:server:packageCount`               | Number of distributable packages a registry server declares                                         | Warning / triage |
| `cdx:registry:server:remoteCount`                | Number of remotely hosted endpoints a registry server declares                                      | Warning / triage |
| `cdx:registry:server:packageRegistryType`        | Registry type (npm/pypi/nuget/oci/mcpb) of a declared distributable                                 | Warning / triage |
| `cdx:registry:server:packageTransport`           | Transport a declared distributable speaks (stdio/streamable-http/sse)                               | Warning / triage |
| `cdx:registry:server:environmentVariableCount`   | Number of environment variables a registry package is handed at launch                              | Warning / triage |
| `cdx:registry:server:runtimeArgumentCount`       | Number of runtime arguments a registry package is launched with                                     | Informational    |
| `cdx:registry:server:packageArgumentCount`       | Number of package arguments a registry package declares                                             | Informational    |
| `cdx:registry:server:secretInputCount`           | Number of secret-typed launch inputs a registry package declares                                    | Hard deny        |
| `cdx:mcp:composition`                            | Marks servers whose composition cannot be verified locally (e.g. registry remotes)                  | Warning / triage |
| `cdx:mcp:protocolVersion`                        | Captures an explicitly pinned MCP protocol revision (e.g. 2026-07-28)                               | Warning / triage |
| `cdx:mcp:deprecatedFeatures`                     | Lists MCP features deprecated by the 2026-07-28 spec that a config still uses                       | Hard deny        |
| `cdx:mcp:auth:cimd`                              | Flags Client ID Metadata Document (CIMD) authorization configuration                                | Warning / triage |
| `cdx:mcp:auth:audienceBound`                     | Marks servers that bind tokens to a specific resource audience per hardened MCP auth                | Warning / triage |
| `cdx:mcpb:manifestVersion`                       | MCPB/DXT bundle manifest schema version                                                             | Informational    |
| `cdx:mcpb:serverType`                            | Runtime type of the server bundled in an MCPB/DXT archive                                            | Warning / triage |
| `cdx:mcpb:toolCount`                             | Number of tools a bundled MCP server exposes                                                        | Warning / triage |
| `cdx:mcpb:promptCount`                           | Number of prompts a bundled MCP server exposes                                                      | Informational    |
| `cdx:mcpb:userConfigFieldCount`                  | Number of declared user-configuration fields in a bundle                                            | Warning / triage |
| `cdx:mcpb:secretFieldCount`                      | Number of secret-typed configuration fields a bundle injects as env vars                            | Hard deny        |
| `cdx:mcpb:injectsSecretEnv`                      | Flags bundles that inject secret configuration values as environment variables                      | Hard deny        |
| `cdx:mcpb:platformBinaryCount`                   | Number of precompiled per-platform binaries a bundle ships                                           | Warning / triage |
| `cdx:ai:quantizationMethod`                      | Non-GGUF quantization method (gptq/awq/exl2/bitsandbytes) distinct from the container format         | Warning / triage |
| `cdx:ai:unsafeDeserialization`                   | Flags pickle-based model artifacts (.bin/.pt/.pth/.ckpt) that execute code on load                  | Hard deny        |
| `cdx:safetensors:tensorCount`                    | Number of tensors declared in a safetensors header                                                  | Informational    |
| `cdx:safetensors:dtypes`                         | Distinct tensor dtypes declared in a safetensors header                                             | Informational    |
| `cdx:agentic:tool`                               | Identifies the discovered agentic CLI tool (kiro-cli, zcode, opencode, claude-code, ...)            | Warning / triage |
| `cdx:agentic:model`                              | Model an agent definition selects (from a tool's agent config)                                      | Warning / triage |
| `cdx:agentic:home`                               | Records the tool's discovered per-user home directory                                               | Informational    |
| `cdx:agentic:supportDir`                         | Records the tool's platform support directory when present                                          | Informational    |
| `cdx:agentic:inventorySource`                    | Marks the component as produced by the host scan (`host-scan`)                                      | Informational    |
| `cdx:agentic:kind`                               | Distinguishes child components: `agent-definition`, `plugin`, or `provider`                         | Warning / triage |
| `cdx:agentic:agentCount`                         | Number of agent definitions the tool declares                                                       | Warning / triage |
| `cdx:agentic:mcpServerCount`                     | Number of MCP servers wired into an agent or plugin                                                 | Warning / triage |
| `cdx:agentic:toolCount`                           | Number of tools an agent definition grants                                                          | Warning / triage |
| `cdx:agentic:allowedToolCount`                   | Number of explicitly allow-listed tools for an agent                                                | Warning / triage |
| `cdx:agentic:pluginCount`                        | Number of installed or cached plugins for the tool                                                  | Warning / triage |
| `cdx:agentic:enabledPluginCount`                 | Number of plugins currently enabled                                                                 | Warning / triage |
| `cdx:agentic:cachedPluginRepoCount`              | Number of cached plugin repositories                                                                | Informational    |
| `cdx:agentic:providerCount`                      | Number of configured inference providers                                                            | Warning / triage |
| `cdx:agentic:hasDefaultModel`                    | Whether a default model is configured                                                               | Informational    |
| `cdx:agentic:knowledgeBaseCount`                 | Number of knowledge bases the tool maintains                                                        | Informational    |
| `cdx:agentic:commandCount`                       | Number of custom slash commands the tool declares                                                   | Informational    |
| `cdx:agentic:skillCount`                         | Number of skills the tool declares                                                                  | Informational    |
| `cdx:agentic:defaultAgent`                       | The configured default agent name                                                                   | Informational    |
| `cdx:agentic:hasSessions`                        | Whether the tool has stored sessions (presence only; transcripts are never read)                   | Informational    |
| `cdx:agentic:license`                            | Declared license of an agentic plugin                                                               | Warning / triage |
| `cdx:agentic:hasSkills`                          | Whether an agentic plugin bundles skills                                                            | Informational    |
...
- **Environment / targeting:** `cdx:npm:cpu`, `cdx:npm:os`, `cdx:npm:libc`, `cdx:pip:markers`, `cdx:python:requires_python`, `cdx:pypi:requiresPython`, `cdx:pixi:operating_system`, `cdx:dotnet:target_framework`, `cdx:dotnet:azure_functions_version`, `cdx:gem:rubyVersionSpecifiers`, `cdx:cargo:rust_version`, `cdx:cargo:rustVersion`, `cdx:cargo:target`
...
| `cdx:npm:packageExtensionsHash`           | root component        | ssri integrity string           | `sha512-...`                                                               | When the lockfile root entry records a `packageExtensionsHash`                                                                                    | Canonical digest of the root `packageExtensions` rule set, comparable to npm's own value                                                                                                                                                                                                              | Warning / triage |
| `cdx:npm:packageExtensionsApplied`        | component             | list string                     | `evil-injected, react`                                                     | When a `packageExtensions` repair changed this dependency's manifest fields                                                                       | Names the dependency edges that the scanned root manifest added to a third-party package, making silent repairs visible                                                                                                                                                                               | Hard deny        |
| `cdx:npm:packageExtensionsDisabled`       | root component        | boolean string                  | `"true"`                                                                   | When `--no-package-extensions` is used with `--deep`                                                                                              | Signals that `packageExtensions` repairs were deliberately skipped so the BOM reflects manifests as published                                                                                                                                                                                         | Context only     |
| `cdx:npm:extensionHash`                   | root component        | ssri integrity string           | `sha512-...`                                                               | When a root `.npm-extension.mjs` or `.npm-extension.cjs` file is detected                                                                         | Hash of the on-disk extension file computed with npm's algorithm (`npm-extension:v1:<format>\n` + bytes), comparable to the lockfile's `npmExtensionHash`                                                                                                                                             | Warning / triage |
| `cdx:npm:extensionFormat`                 | root component        | enum string                     | `"mjs"`, `"cjs"`                                                           | When a root `.npm-extension` file is detected                                                                                                     | Module format of the detected extension file                                                                                                                                                                                                                                                          | Context only     |
| `cdx:npm:extensionApplied`                | root component        | enum string                     | `"true"`, `"false"`, `"unverified"`                                        | When a root `.npm-extension` file is detected                                                                                                     | Whether the extension's repairs are reflected: `true` (on-disk hash matches the lockfile), `false` (lockfile records no extension), `unverified` (the file changed since the lockfile was written)                                                                                                    | Hard deny        |
| `cdx:npm:extensionFieldsApplied`          | component             | list string                     | `left-pad, react`                                                          | When a `.npm-extension` repair changed this dependency's manifest fields                                                                          | Names the dependency edges that the root `.npm-extension` added or corrected on a third-party package                                                                                                                                                                                                 | Warning / triage |
...
| `cdx:pypi:classifiers`                    | component             | list string (newline separated) | `Framework :: Flask\nTopic :: Internet :: WWW/HTTP :: WSGI :: Application` | When PyPI metadata is fetched and the distribution declares Trove classifiers                                                                     | Lets policies read the full author-declared taxonomy rather than only the tags cdxgen maps                                                                                                                                                                                                            | Context only     |
| `cdx:pypi:frameworks`                     | component             | list string (comma separated)   | `django,celery`                                                            | When a distribution declares one or more `Framework :: X` classifiers                                                                             | Authoritative framework attribution; more reliable than name-prefix matching for plugin packages                                                                                                                                                                                                      | Context only     |
| `cdx:pypi:modulesFrom`                    | component             | enum string                     | `alias-map,distribution-name`, `distribution-name`                         | Alongside `internal:Namespaces` on PyPI components                                                                                                | Records whether the module names were curated or derived from the distribution name                                                                                                                                                                                                                   | Context only     |
...
| `cdx:gem:extensions`                      | component             | comma separated string          | `ext/nokogiri/extconf.rb`                                                  | When a gemspec declares native extensions                                                                                                         | The gem compiles code on the install host, the RubyGems equivalent of an npm install script                                                                                                                                                                                                           | Warning / triage |
| `cdx:gem:mfaRequired`                     | component             | boolean string                  | `"true"`, `"false"`                                                        | When the gemspec metadata or registry metadata sets `rubygems_mfa_required`                                                                       | Indicates the maintainer requires MFA to publish, a positive supply chain signal                                                                                                                                                                                                                      | Informational    |
| `cdx:gem:rubygemsVersionSpecifiers`       | component             | string                          | `>= 3.3.22`                                                                | From `required_rubygems_version` in a gemspec, or the `rubygems:` field of the bundler compact index cache                                        | Build and install compatibility signal                                                                                                                                                                                                                                                                | Informational    |
...
| `cdx:dotnet:output_type`                  | component             | string                          | `Exe`, `WinExe`                                                            | When an MSBuild project declares a non-`Library` `OutputType`                                                                                     | Distinguish executable projects from libraries when scoping reachability or runtime-exposure policy. Emitted as a property rather than a purl qualifier: `nuget` does not define `output_type`, so the previous `?output_type=` purl was invalid. `Library` is reflected in `component.type` instead. | Context only     |
| `cdx:purl:sanitized_from`                 | component             | purl string                     | `pkg:generic/x@1?arch=arm64`                                               | When cdxgen ingests a component whose purl came from external metadata and was not a valid Package URL                                            | Detect upstream tools emitting invalid purls; audit where cdxgen repaired or dropped identity it did not author. The original, invalid purl. cdxgen rebuilds a canonical purl from the component identity where possible, otherwise drops the purl and assigns a fallback `bom-ref`.                  | Warning / triage |
| `cdx:caxa:arch`                           | component             | string                          | `arm64`, `x64`                                                             | On the parent component of a caxa-packaged standalone binary (`-t caxa`)                                                                          | Confirm a shipped binary matches the intended target architecture. Emitted by `@cdxgen/caxa` and ingested by cdxgen. Previously an `arch` purl qualifier, which the `generic` type does not permit.                                                                                                   | Context only     |
| `cdx:caxa:platform`                       | component             | string                          | `linux`, `darwin`, `win32`                                                 | On the parent component of a caxa-packaged standalone binary (`-t caxa`)                                                                          | Confirm a shipped binary matches the intended target platform. Emitted by `@cdxgen/caxa` and ingested by cdxgen. Previously a `platform` purl qualifier, which the `generic` type does not permit.                                                                                                    | Context only     |
...
<a id="rusi-rust-evinse-evidence"></a>
...
- There is no `cdx:gem:platform` property. RubyGems models the version and the native platform separately, so the platform is emitted as the `platform` purl qualifier, for example `pkg:gem/nokogiri@1.16.7?platform=x86_64-linux`. A policy can read it with `$substringAfter(purl, 'platform=')`, and the absence of the qualifier means the implied default platform `ruby`.
...
| `cdx:dosai:operationCount`                 | service   | number string  | `1`, `6`                                      | When dosai reports operations for a provider service             | Sizes the operation surface behind a service beyond its endpoints   | Context only     |
| `cdx:dosai:authScheme`                     | service   | list string    | `JwtBearer`, `Cookie`                         | When dosai detects authentication schemes on a service           | Distinguishes authenticated surfaces from anonymous ones            | Warning / triage |
...
Build introspection carries its verdict inside the BOM so a consumer who
receives only the document still learns how much to trust it. The properties
are emitted on `metadata.properties` when generation runs with `--introspect`,
`--profile introspect`, or `CDXGEN_INTROSPECT=true`. The overall `tier` and
...
- `cdx:introspection:ledgerComplete` — `true` when the build ledger covered the whole run, `false` when it was truncated.
...
a record of _this_ run, which is how a later re-scan of the BOM tells the
...
- `cdx:introspection:remediationId` — Catalog or build-fidelity rule id the remediation is keyed by.
- `cdx:introspection:ecosystem` — Ecosystem the remediation improves.
- `cdx:introspection:currentTier` — Tier the ecosystem holds today.
- `cdx:introspection:targetTier` — Tier the fix is expected to reach.
- `cdx:introspection:expectedGain` — Expected overall score gain, two decimals.
- `cdx:introspection:confidence` — Confidence label for the remediation's evidence (`high`, `medium`, `low`).
- `cdx:introspection:verify:rules` — Comma-separated build-fidelity rule ids that must stop firing once the fix is applied.
- `cdx:introspection:evidence:<key>` — Per-fact evidence entries (`source`, `impact`, `severity`, `events`, `subsumes`, `eventsCleared`).
