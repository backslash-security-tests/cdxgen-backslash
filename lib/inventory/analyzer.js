...
import { mapInWorkers } from "../core/parallel.js";
import {
...
const IGNORE_DIRS = readEnvironmentVariable("ASTGEN_IGNORE_DIRS")
  ? readEnvironmentVariable("ASTGEN_IGNORE_DIRS").split(",")
  : [
      "venv",
      "docs",
      "test",
...
const _astWorkerUrl = new URL("./astWorker.js", import.meta.url);
...
const normalizeAnalyzerSearchOptions = (deepOrOptions = false) => {
  if (deepOrOptions && typeof deepOrOptions === "object") {
    return {
      ...
      noIgnore: Boolean(deepOrOptions.noIgnore),
...
const getAllFiles = (
  ...
  for (let i = 0; i < files.length; i++) {
    if (
      !noIgnore &&
      (IGNORE_FILE_PATTERN.test(files[i]) || files[i].startsWith("."))
    ) {
      continue;
    }
    ...
    if (fileStat.isDirectory()) {
      ...
      if (!noIgnore && !deep && dirName === "node_modules") {
        continue;
      }
      try {
        result = getAllFiles(
...
/**
 * Record a usage occurrence for an imported binding at a specific call site.
 * This provides lightweight call tracking (method calls and direct invocations
 * of an imported binding) beyond the import declaration itself, without
 * requiring a full data-flow slice from atom. The occurrence is keyed by the
 * original module specifier so it lines up with the import-declaration
 * occurrences already collected in `allImports`.
 *
 * @param {Object} allImports Accumulated imports map.
 * @param {string} src Project root.
 * @param {string} file Absolute file path being analyzed.
 * @param {string} moduleFullPath Module specifier the binding was imported from.
 * @param {Object} loc Babel source location (`{line, column}`).
 */
const addUsageOccurrence = (allImports, src, file, moduleFullPath, loc) => {
  if (!moduleFullPath) {
    return;
  }
  const occurrence = {
    importedAs: moduleFullPath,
    importedModules: [],
    exportedModules: [],
    isExternal: true,
...
const vueCleaningRegex =
  /<\/?script[^>]*>|<style[^>]*>[\s\S]*?<\/style\s*>|<\/?br\s*\/?>/gi;
...
const fileToParseableCode = (file) => {
  ...
  if (file.endsWith(".vue") || file.endsWith(".svelte")) {
    code = code
      ...
      // Strip the entire <template> block content to spaces (preserving newlines
      // for accurate line numbers). Vue-specific binding attributes such as
      // `:prop-name`, `@update:model-value` and `v-*` directives are not valid
      // JSX and cause Babel to throw even when errorRecovery is enabled, so the
      // template HTML must be blanked out before AST parsing.
      .replace(
        vueTemplateRegex,
        (_match, grA, grB, grC) =>
          grA.replaceAll(/\S/g, " ") +
...
const ANGULAR_INLINE_TEMPLATE_METADATA_KEYS = new Set(["template"]);

const ANGULAR_ICON_METADATA_KEYS = new Set([
  "collapsedIcon",
  "expandedIcon",
  "icon",
]);
...
const ANGULAR_STYLE_FILE_EXTENSIONS = [".css", ".less", ".sass", ".scss"];

const ANGULAR_STYLE_REFERENCE_PATTERNS = [
...
const VUE_CONFIG_FILE_NAMES = new Set([
  "vite.config.cjs",
  "vite.config.js",
  "vite.config.mjs",
  "vite.config.mts",
...
const ANGULAR_SCRIPT_EXECUTABLE_PACKAGE_MAP = {
...
const ANGULAR_PACKAGE_MANAGER_WRAPPER_COMMANDS = new Set([
  "npx",
  "pnpm",
  "yarn",
]);

const ANGULAR_EXECUTION_WRAPPER_COMMANDS = new Set(["node"]);

const angularScriptSegmentSplitRegex = /&&|\|\||;|\|/;
const angularShellTokenRegex =
  /"[^"\\]*(?:\\.[^"\\]*)*"|'[^'\\]*(?:\\.[^'\\]*)*'|\S+/g;
const angularShellEnvAssignRegex = /^[A-Za-z_][A-Za-z0-9_]*=/;
...
const ANGULAR_TEMPLATE_PACKAGE_PATTERNS = [
  ...
  },
  {
    ...
    pattern:
      /\bclass\s*=\s*["'][^"']*\b(?:fa|fas|far|fab|fa-solid|fa-regular|fa-brands)\b[^"']*\bfa-[\w-]+\b|\b(?:fa|fas|far|fab|fa-solid|fa-regular|fa-brands)\s+fa-[\w-]+\b/,
  },
  {
  ...
  },
  {
    ...
    pattern:
      /\bclass\s*=\s*["'][^"']*\bbi\b[^"']*\bbi-[\w-]+\b|\bbi\s+bi-[\w-]+\b/,
  },
  {
    packageName: "material-symbols",
    pattern:
      /\bfontSet\s*=\s*["'][^"']*material-symbols[^"']*["']|\bclass\s*=\s*["'][^"']*material-symbols[^"']*["']/,
  },
  {
    packageName: "primeicons",
    pattern:
      /\bclass\s*=\s*["'][^"']*\bpi\b[^"']*\bpi-[\w-]+\b|\bpi\s+(?:pi-fw\s+)?pi-[\w-]+\b/,
  },
  {
...
const angularBareFileRegex =
  /^[\w.-]+\.(css|eot|gif|html|ico|jpeg|jpg|js|json|less|mjs|png|sass|scss|svg|ts|ttf|webp|woff|woff2)$/i;

const isAngularStyleUrlPackageReference = (reference) => {
...
const stripQuotedShellToken = (token) =>
  String(token || "")
    .trim()
    .replace(/^['"]|['"]$/g, "");

const getExecutableNameFromPathLikeToken = (token) => {
  let normalizedToken = stripQuotedShellToken(token)
    .replaceAll("\\", "/")
    .split(/[?#]/)[0]
    .trim();
  if (!normalizedToken) {
  ...
  if (normalizedToken.startsWith("@")) {
    const scopedPackageMatch = normalizedToken.match(/^(@[^/\s]+\/[^/\s]+)/);
    return scopedPackageMatch?.[1];
  }
  if (normalizedToken.startsWith("node_modules/.bin/")) {
    normalizedToken = normalizedToken.slice("node_modules/.bin/".length);
  }
  if (normalizedToken.includes("/")) {
    normalizedToken = basename(normalizedToken);
  }
  return normalizedToken.replace(/\.(cjs|cts|js|jsx|mjs|mts|ts|tsx)$/i, "");
};

const extractScriptSegmentExecutableName = (scriptSegment) => {
  const tokens = (
    String(scriptSegment || "").match(angularShellTokenRegex) || []
  )
    .map((token) => stripQuotedShellToken(token))
    .filter(Boolean);
  if (!tokens.length) {
    return undefined;
  }
  ...
  while (
    index < tokens.length &&
    angularShellEnvAssignRegex.test(tokens[index])
  ) {
    index += 1;
  }
  ...
  if (commandToken === "npm") {
    if (["exec", "x"].includes(tokens[index + 1])) {
      return {
        executableName: getExecutableNameFromPathLikeToken(tokens[index + 2]),
        packageHintOnly: true,
      };
    }
    return undefined;
  }
  if (ANGULAR_PACKAGE_MANAGER_WRAPPER_COMMANDS.has(commandToken)) {
    if (["exec", "dlx"].includes(tokens[index + 1])) {
      return {
        executableName: getExecutableNameFromPathLikeToken(tokens[index + 2]),
        packageHintOnly: true,
      };
    ...
    if (commandToken === "npx") {
      return {
        executableName: getExecutableNameFromPathLikeToken(tokens[index + 1]),
        packageHintOnly: true,
      };
    }
    return undefined;
  }
  if (ANGULAR_EXECUTION_WRAPPER_COMMANDS.has(commandToken)) {
    for (
      ...
      if (value.startsWith("node_modules/.bin/") || value.includes("/.bin/")) {
        return {
          executableName: getExecutableNameFromPathLikeToken(value),
          packageHintOnly: false,
        };
      }
      return undefined;
    }
  ...
  return {
    executableName: getExecutableNameFromPathLikeToken(commandToken),
    packageHintOnly: false,
  };
};

const addAngularExecutablePackageRef = (
  allImports,
  allExports,
  src,
  file,
  ...
  const executableName = executable?.executableName;
  if (!executableName) {
    return;
  }
  const packageName =
    ANGULAR_SCRIPT_EXECUTABLE_PACKAGE_MAP[executableName.toLowerCase()];
  if (packageName) {
  ...
  // For npx/npm exec/pnpm dlx/yarn dlx, executable token usually maps to a package.
  if (!executable.packageHintOnly) {
    return;
  }
...
const addAngularTemplatePatternPackageRefs = (
  allImports,
  allExports,
  src,
  file,
  ...
  for (const [index, line] of lines.entries()) {
    for (const templatePattern of ANGULAR_TEMPLATE_PACKAGE_PATTERNS) {
      templatePattern.pattern.lastIndex = 0;
      if (templatePattern.pattern.test(line)) {
        setAngularPackageRef(
          ...
          baseSourceLoc
            ? {
                column: index === 0 ? baseSourceLoc.column : 0,
                line: baseSourceLoc.line + index,
              }
            : { column: 0, line: index + 1 },
        );
      }
    }
  }
...
const parseFileASTTree = (src, file, allImports, allExports) => {
  ...
  // Map of local binding name -> external module specifier, used to attribute
  // method calls / invocations back to the imported package (lightweight call
  // tracking). Only external (non-relative) imports are tracked.
  const bindingModules = new Map();
  traverse(ast, {
      ...
      if (path?.node) {
        ...
        if (
          ...
          for (const specifier of path.node.specifiers || []) {
            ...
            if (localName) {
              bindingModules.set(localName, importSource);
            }
          }
        }
        const isTypeOnly =
    ...
    CallExpression: (path) => {
      ...
      // Lightweight call tracking: record a usage occurrence at each site where
      // an imported binding is invoked directly or via a method call, so the
      // package's evidence covers use sites and not just the import statement.
      const callee = path?.node?.callee;
      if (callee?.type === "Identifier" && bindingModules.has(callee.name)) {
        addUsageOccurrence(
          allImports,
          src,
          file,
          bindingModules.get(callee.name),
          path.node.loc?.start,
        );
      } else if (
        callee?.type === "MemberExpression" &&
        callee.object?.type === "Identifier" &&
        bindingModules.has(callee.object.name)
      ) {
        addUsageOccurrence(
          allImports,
          src,
          file,
          bindingModules.get(callee.object.name),
          path.node.loc?.start,
        );
      }
    ...
    ObjectProperty: (path) => {
      ...
      if (
        (ANGULAR_INLINE_TEMPLATE_METADATA_KEYS.has(keyName) ||
          ANGULAR_ICON_METADATA_KEYS.has(keyName)) &&
        Object.keys(allImports).some((importName) =>
          isAngularPackageSignal(importName),
        )
      ) {
        for (const stringNode of getAngularStringNodes(path.node.value)) {
          const content = getStringValue(stringNode);
          addAngularTemplatePatternPackageRefs(
            allImports,
            allExports,
            src,
            file,
    ...
    ExportAllDeclaration: (path) => {
      const isTypeOnly =
        path.node.exportKind === "type" || path.node.exportKind === "typeof";
      setFileRef(
        allImports,
        allExports,
        src,
 ...
 */
export const parseFileASTTreeCollected = (src, file) => {
  const imports = {};
  const exports = {};
  parseFileASTTree(src, file, imports, exports);
...
const parseAngularStyleFiles = (
  ...
  for (const extension of ANGULAR_STYLE_FILE_EXTENSIONS) {
    const styleFiles = getAllFiles(
      deep,
      src,
      extension,
      undefined,
    ...
    for (const styleFile of styleFiles) {
      let content;
      try {
        content = readFileSync(styleFile, "utf-8");
      } catch {
        continue;
      }
      for (const referencePattern of ANGULAR_STYLE_REFERENCE_PATTERNS) {
        for (const match of content.matchAll(referencePattern)) {
          ...
          if (
            ...
            !isAngularStyleUrlPackageReference(reference)
          ) {
            continue;
          }
          ...
          const columnNumber =
            lineStart >= 0 ? prefix.length - lineStart - 1 : 0;
          setAngularPackageRef(
            allImports,
            allExports,
            src,
...
const addAngularScriptPackageRefs = (
  ...
  const scriptSegments = script
    .split(angularScriptSegmentSplitRegex)
    .map((segment) => segment.trim())
    .filter(Boolean);
  for (const scriptSegment of scriptSegments) {
    const executable = extractScriptSegmentExecutableName(scriptSegment);
    addAngularExecutablePackageRef(
      allImports,
      allExports,
      src,
      file,
...
const getVueConfigFiles = (
  ...
  for (const entry of files) {
    ...
    if (VUE_CONFIG_FILE_NAMES.has(entry)) {
      result.push(file);
    }
  }
  return result;
...
const parseVueConfigFiles = (
  ...
  const configFiles = getVueConfigFiles(
    src,
    deep,
    [],
    src,
  ...
  for (const configFile of configFiles) {
    ...
    // Extract package references from CSS preprocessor additionalData strings.
    // additionalData is used to globally inject SCSS/Less imports into every
    // processed style file, so packages referenced there may not appear anywhere
    // else in the project source, causing false-positive optional scope.
    const additionalDataRegex =
      /additionalData\s*:\s*(?:`([^`]*)`|"((?:[^"\\]|\\.)*)"|'((?:[^'\\]|\\.)*)')/g;
    for (const additionalDataMatch of content.matchAll(additionalDataRegex)) {
      const dataValue =
        additionalDataMatch[1] ??
        additionalDataMatch[2] ??
        additionalDataMatch[3];
      if (!dataValue) {
        continue;
      }
      for (const referencePattern of ANGULAR_STYLE_REFERENCE_PATTERNS) {
        referencePattern.lastIndex = 0;
        for (const styleMatch of dataValue.matchAll(referencePattern)) {
          const reference = styleMatch[1];
          if (!reference) {
            continue;
          }
          if (
            styleMatch[0]?.trim().startsWith("url") &&
            !isAngularStyleUrlPackageReference(reference)
          ) {
            continue;
          }
...
/**
 * Total source volume, in bytes, below which the file walk stays on the calling
 * thread even when the file count would allow threads.
 *
 * Parse cost tracks bytes far better than it tracks file count. Measured on a
 * 14-core machine: 400 one-line files total 17 KiB and parse in 23 ms serially,
 * while starting the pool costs about 85 ms, so threading that tree is a pure
 * loss. The same machine parses 3.7 MiB of cdxgen's own sources in 1036 ms
 * serially and 507 ms across 8 workers. Gating on volume keeps the win on trees
 * that have work to divide and keeps the startup cost off trees that do not.
 */
const MIN_PARALLEL_PARSE_BYTES = 1024 * 1024;

/**
 * Total size of the given files, skipping any that cannot be stat'ed.
 *
 * @param {Array<string>} files Absolute file paths.
 * @returns {number} Sum of sizes in bytes.
 */
const totalSourceBytes = (files) => {
  let bytes = 0;
  for (const file of files) {
    try {
      bytes += statSync(file).size;
    } catch (_err) {
      // A file that disappeared between the walk and here contributes nothing.
    }
  }
  return bytes;
};
...
/**
 * Merge per-file collected results into shared objects in a deterministic order.
 * Iterates source keys in insertion order and appends to target Sets.
 */
const mergeCollectedResults = (target, source) => {
  for (const key of Object.keys(source)) {
    const srcSet = source[key];
    if (srcSet === undefined) continue;
    if (!target[key]) {
      target[key] = new Set(srcSet);
    } else {
      for (const occ of srcSet) {
        target[key].add(occ);
      }
    }
  }
...
export const findJSImportsExports = async (src, deep) => {
  ...
  try {
    ...
    const tasks = srcFiles.map((file) => ({ src, file }));
    const parallelWorthwhile =
      totalSourceBytes(srcFiles) >= MIN_PARALLEL_PARSE_BYTES;
    const fileResults = await mapInWorkers(
      _astWorkerUrl,
      tasks,
      (task) => parseFileASTTreeCollected(task.src, task.file),
      {
        swallowErrors: true,
        errorResult: null,
        threshold: parallelWorthwhile ? undefined : Number.POSITIVE_INFINITY,
      },
    );
    for (const result of fileResults) {
      ...
      mergeCollectedResults(allImports, result.imports);
      mergeCollectedResults(allExports, result.exports);
    }
...
const MCP_APP_FACTORIES = new Set(["createMcpExpressApp", "createMcpHandler"]);
...
const MCP_PROVIDER_HOST_PATTERNS = [
  ...
  ["cerebras", /(?:^|\.)api\.cerebras\.ai$/i],
  ["dashscope", /(?:^|\.)dashscope(?:-intl)?\.aliyuncs\.com$/i],
  ["nvidia-nim", /(?:^|\.)integrate\.api\.nvidia\.com$/i],
];
...
const registerCapabilityObject = (serviceInfo, capabilitiesNode) => {
  if (capabilitiesNode?.type !== "ObjectExpression") {
    return;
  }
...
const primitiveComponentForMcp = (serviceInfo, primitive) => {
  ...
  const properties = [
    { name: "internal:SrcFile", value: serviceInfo.file },
...
const serviceObjectForMcp = (serviceInfo) => {
  ...
  const properties = [{ name: "internal:SrcFile", value: serviceInfo.file }];
