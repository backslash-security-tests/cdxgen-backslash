...
import { githubActionsParser } from "../../inventory/ciParsers/githubActions.js";
import { createLolbasProperties } from "../../inventory/lolbas.js";
import {
  auditBom,
  formatAnnotations,
...
function makeChromeExtensionComponent(name, version, properties) {
  const purl = `pkg:chrome-extension/${name}@${version}`;
  return {
    type: "application",
    name,
    version,
...
describe("loadRules", () => {
  ...
  it("never feeds an unguarded property into a null-intolerant function", async () => {
    // $prop yields null for an absent property, and these JSONata builtins
    // reject null with a signature error rather than returning nothing. The
    // whole rule then fails to evaluate for every component that happens to
    // lack the property, so the rule stops firing on the components that do
    // carry it. JSONata short-circuits `and`, so an earlier existence test in
    // the same chain is a valid guard; without one, read through
    // $nullSafeProp.
    const NULL_INTOLERANT = [
      "contains",
      "substringBefore",
      "substringAfter",
      "split",
    ...
    const unguarded = new RegExp(
      `\\$(${NULL_INTOLERANT})\\(\\$prop\\(\\$, *'([^']+)'`,
      "g",
    );
    const rules = await loadRules(RULES_DIR);
    const violations = [];
    for (const rule of rules) {
      for (const [field, expression] of Object.entries(rule)) {
        if (typeof expression !== "string") {
          continue;
        }
        for (const match of expression.matchAll(unguarded)) {
          const [, fn, property] = match;
          const guarded = [
            `$hasProp($, '${property}')`,
            `$safeStr($prop($, '${property}'))`,
            `$nullSafeProp($, '${property}')`,
          ].some((guard) => {
            const at = expression.indexOf(guard);
            return at !== -1 && at < match.index;
          });
          if (!guarded) {
            violations.push(
              `${rule.id} passes '${property}' into $${fn} in ${field}`,
            );
          }
        }
      }
    ...
    assert.deepStrictEqual(
      violations,
      [],
      `Read these through $nullSafeProp or test the property first: ${violations.join("; ")}`,
    );
  });
...
describe("evaluateRule", () => {
  ...
  it("should detect Golem crypto data-flow evidence (GOLEM-SEC-002)", async () => {
    ...
    const bom = makeBom([
      makeComponent("github.com/acme/app", "1.0.0", [
        ...
        ["cdx:golem:cryptoDataFlowCategories", "http-input->crypto"],
        ...
        properties: [
          {
            ...
            value: "/repo/prompts/release-prompt.yaml",
          },
      ...
      [
        {
          ...
          authenticated: false,
          properties: [
            { name: "internal:SrcFile", value: "src/unsafe.js" },
      ...
      [
        {
          ...
          authenticated: true,
          properties: [
            { name: "internal:SrcFile", value: "src/custom.js" },
            { name: "cdx:mcp:transport", value: "streamable-http" },
            { name: "cdx:mcp:officialSdk", value: "false" },
      ...
      [
        {
          ...
          type: "file",
          properties: [
            { name: "internal:SrcFile", value: "/repo/AGENTS.md" },
      ...
      [
        {
          ...
          authenticated: false,
          properties: [
            { name: "internal:SrcFile", value: "/repo/.vscode/mcp.json" },
    ...
    const bom = makeBom([
      {
        ...
        type: "file",
        properties: [
          { name: "internal:SrcFile", value: "/repo/CLAUDE.md" },
  ...
  it("should flag deprecated MCP protocol features (MCP-009)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "MCP-009");
    assert.ok(rule, "MCP-009 rule should exist");
    ...
    const bom = makeBom(
      ...
      [
        {
          "bom-ref": "urn:service:mcp:legacy:latest",
          name: "legacy",
          group: "mcp",
          endpoints: ["https://legacy.example.com/sse"],
          properties: [
            { name: "internal:SrcFile", value: "/repo/.mcp.json" },
            { name: "cdx:mcp:transport", value: "sse" },
            {
              name: "cdx:mcp:deprecatedFeatures",
              value: "dynamic-client-registration,sse-transport",
            },
          ],
        },
      ],
    ...
    const findings = await evaluateRule(rule, bom);
    assert.ok(findings.length > 0, "Should detect deprecated MCP features");
    assert.strictEqual(findings[0].severity, "medium");
  });

  it("should flag MCP bundles that inject secret env (MCP-010)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "MCP-010");
    assert.ok(rule, "MCP-010 rule should exist");

    const bom = makeBom([
      {
        "bom-ref": "urn:mcp:bundle:weather:weather.mcpb",
        type: "application",
        name: "weather-extension",
        properties: [
          { name: "internal:SrcFile", value: "/repo/weather.mcpb" },
          { name: "cdx:file:kind", value: "mcp-bundle" },
          { name: "cdx:mcpb:injectsSecretEnv", value: "true" },
          { name: "cdx:mcpb:secretFieldCount", value: "1" },
        ],
      },
    ]);

    const findings = await evaluateRule(rule, bom);
    assert.ok(findings.length > 0, "Should detect secret-injecting bundle");
    assert.strictEqual(findings[0].severity, "low");
  });

  it("should flag MCP bundles that ship platform binaries (MCP-011)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "MCP-011");
    assert.ok(rule, "MCP-011 rule should exist");

    const bom = makeBom([
      {
        "bom-ref": "urn:mcp:bundle:native:native.mcpb",
        type: "application",
        name: "native-extension",
        properties: [
          { name: "internal:SrcFile", value: "/repo/native.mcpb" },
          { name: "cdx:file:kind", value: "mcp-bundle" },
          { name: "cdx:mcpb:platformBinaryCount", value: "3" },
        ],
      },
    ]);

    const findings = await evaluateRule(rule, bom);
    assert.ok(findings.length > 0, "Should detect bundled platform binaries");
    assert.strictEqual(findings[0].severity, "medium");
  });

  it("should flag unsafe pickle model artifacts (AIS-003)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "AIS-003");
    assert.ok(rule, "AIS-003 rule should exist");

    const bom = makeBom([
      makeAiModel("pytorch_model", "1.0.0", [
        ["cdx:ai:artifactFormat", "pytorch-pickle"],
        ["cdx:ai:unsafeDeserialization", "true"],
        ["cdx:ai:source", "local-artifact"],
      ]),
    ]);

    const findings = await evaluateRule(rule, bom);
    assert.ok(findings.length > 0, "Should detect unsafe pickle model");
    assert.strictEqual(findings[0].severity, "high");
  });
      ...
      [
        {
          ...
          properties: [
            { name: "internal:SrcFile", value: "/tmp/Cargo.toml" },
  ...
  it("stays evaluable for a native Cargo project with no capability properties (INT-013)", async () => {
    const rules = await loadRules(RULES_DIR);
    const rule = rules.find((r) => r.id === "INT-013");
    const bom = makeBom(
      [],
      ...
      [
        {
          ...
          name: "cargo-plain",
          version: "config",
          "bom-ref": "urn:cdxgen:formulation:cargo:int013-plain",
          properties: [
            { name: "internal:SrcFile", value: "/tmp/Cargo.toml" },
            { name: "cdx:rust:buildTool", value: "cargo" },
            { name: "cdx:cargo:hasNativeBuild", value: "true" },
    ...
    // The engine reports an unevaluable rule by warning and returning no
    // findings, so the warning is the only evidence the condition threw.
    // Other warnings reach the same stream — the sandbox refuses to run git
    // on the hosted Windows runners — so only the evaluation failure counts.
    const failures = [];
    const originalWarn = console.warn;
    console.warn = (...args) => {
      const text = args.join(" ");
      if (text.includes("Failed to evaluate rule")) {
        failures.push(text);
      }
    };
    try {
    ...
    const bom = makeBom([
      {
        ...
        properties: [
          ...
          { name: "internal:SrcFile", value: "/tmp/app.asar" },
  ...
  it("should detect embedded install scripts from ASAR-derived npm components (ASAR-004)", async () => {
    ...
    const bom = makeBom([
      makeComponent("sketchy-addon", "0.1.0", [
        ["internal:SrcFile", "/tmp/app.asar#/package-lock.json"],
        ["cdx:npm:hasInstallScript", "true"],
      ...
      [
        {
          ...
          name: "README.md",
          properties: [
            { name: "internal:SrcFile", value: "README.md" },
    ...
    const bom = makeBom([
      {
        ...
        properties: [
          { name: "internal:SrcFile", value: "/bin/bash" },
    ...
    const bom = makeBom([
      {
        ...
        properties: [
          { name: "internal:SrcFile", value: "/usr/bin/docker" },
          { name: "cdx:gtfobins:matched", value: "true" },
    ...
    const bom = makeBom([
      {
        ...
        "bom-ref": "pkg:generic/bash",
        properties: [
          { name: "internal:SrcFile", value: "/usr/bin/bash" },
    ...
    const bom = makeBom([
      {
        ...
        "bom-ref": "pkg:generic/bash",
        properties: [
          { name: "internal:SrcFile", value: "/usr/local/bin/bash" },
          { name: "cdx:gtfobins:matched", value: "true" },
          { name: "cdx:gtfobins:name", value: "bash" },
    ...
    const bom = makeBom([
      {
        ...
        properties: [
          { name: "internal:SrcFile", value: "/usr/local/bin/deepce" },
    ...
    const bom = makeBom([
      {
        ...
        properties: [
          { name: "internal:SrcFile", value: "/usr/bin/nsenter" },
          { name: "cdx:container:matched", value: "true" },
  ...
  // CTR-009/CTR-010 evaluate `$hasSbom`/`$hasSignedSbom`, which probe a live OCI
  // registry (getBomWithOras) from inside JSONata. On network-restricted CI
  // runners the probe still exceeds the 5s JSONata evaluation budget (the
  // per-request timeouts help but the runner can blackhole the connection past
  // the budget), yielding no findings and flaky failures. Skipped to keep CI
  // deterministic; run locally with network access to validate registry
  // integration. The registry-probe timeout plumbing itself is covered by the
  // httpClient/oci unit tests.
  it.skip("should detect container dependency lacking attached SBOM (CTR-009)", async () => {
    const prev = process.env.CDXGEN_OCI_TIMEOUT_MS;
    process.env.CDXGEN_OCI_TIMEOUT_MS = "1500";
    try {
      const rules = await loadRules(RULES_DIR);
      const rule = rules.find((r) => r.id === "CTR-009");
      assert.ok(rule, "CTR-009 rule should exist");

      // Point at ghcr.io (responds faster than docker.io for missing images).
      const bom = makeBom([
        {
          type: "container",
          name: "dummy-nonexistent-image",
          version: "1.0.0",
          purl: "pkg:oci/dummy-nonexistent-image@1.0.0?repository_url=ghcr.io/cdxgen/dummy-nonexistent-image",
          "bom-ref":
            "pkg:oci/dummy-nonexistent-image@1.0.0?repository_url=ghcr.io/cdxgen/dummy-nonexistent-image",
        },
      ]);
      ...
      if (prev === undefined) {
        delete process.env.CDXGEN_OCI_TIMEOUT_MS;
      } else {
        process.env.CDXGEN_OCI_TIMEOUT_MS = prev;
      }
    }
  });

  it.skip("should detect container dependency lacking signed SBOM (CTR-010)", async () => {
    const prev = process.env.CDXGEN_OCI_TIMEOUT_MS;
    process.env.CDXGEN_OCI_TIMEOUT_MS = "1500";
    try {
      const rules = await loadRules(RULES_DIR);
      ...
      const bom = makeBom([
        {
          ...
          version: "1.0.0",
          purl: "pkg:oci/dummy-nonexistent-image-unsigned@1.0.0?repository_url=ghcr.io/cdxgen/dummy-nonexistent-image-unsigned",
          "bom-ref":
            "pkg:oci/dummy-nonexistent-image-unsigned@1.0.0?repository_url=ghcr.io/cdxgen/dummy-nonexistent-image-unsigned",
        },
      ]);
...
describe("evaluateRules", () => {
  it("forwards injected helpers to each rule (hermetic, no registry)", async () => {
    // `getBomWithOras` is injected from a layer above stages and reaches
    // JSONata's $hasSbom/$hasSignedSbom through the options bag. evaluateRules
    // once accepted `options` and dropped it on the way to evaluateRule, which
    // silently made every registry-probing rule evaluate to false.
    const rules = await loadRules(RULES_DIR);
    const ctr = rules.filter((r) => ["CTR-009", "CTR-010"].includes(r.id));
    assert.ok(ctr.length > 0, "registry-probing rules should exist");
    let calls = 0;
    const getBomWithOras = async () => {
      calls++;
      return undefined;
    };
    const bom = makeBom([
      {
        ...
        name: "probe-image",
        version: "1.0.0",
        purl: "pkg:oci/probe-image@1.0.0?repository_url=ghcr.io/cdxgen/probe-image",
        "bom-ref":
          "pkg:oci/probe-image@1.0.0?repository_url=ghcr.io/cdxgen/probe-image",
      },
    ]);
    await evaluateRules(ctr, bom, { getBomWithOras });
    assert.ok(calls > 0, "injected getBomWithOras should have been invoked");
  });
...
describe("auditBom", () => {
  ...
  it("reports active dry-run support counts for package-integrity rules", async () => {
    ...
    assert.match(
      ...
      /3 rule\(s\) do not support dry-run, 1 rule\(s\) have partial dry-run support, 15 active rule\(s\) total/,
    );
  });
...
describe("additional OBOM and rootfs hardening rules", () => {
    ...
    const bom = makeBom([
      {
        ...
        properties: [
          { name: "internal:SrcFile", value: "/etc/yum.repos.d/internal.repo" },
      ...
      [
        {
          ...
          properties: [
            {
              name: "internal:SrcFile",
              value: "/etc/systemd/system/evil.service",
            },
  ...
  it("should evaluate CBOM rules (CBS-001 through CBS-004 and CBC-001)", async () => {
    const rules = await loadRules(RULES_DIR);

    // CBS-001 Weak/Deprecated Algorithm (MD5)
    const rule1 = rules.find((r) => r.id === "CBS-001");
    assert.ok(rule1, "CBS-001 rule should exist");
    const bomMD5 = makeBom([
      {
        type: "cryptographic-asset",
        name: "md5-hasher",
        version: "1.0.0",
        purl: "pkg:npm/md5-hasher@1.0.0",
        "bom-ref": "pkg:npm/md5-hasher@1.0.0",
        cryptoProperties: {
          assetType: "algorithm",
          algorithmProperties: {
            algorithmFamily: "MD5",
    ...
    const findings1 = await evaluateRule(rule1, bomMD5);
    assert.ok(findings1.length > 0, "Should detect MD5 algorithm as weak");
    assert.strictEqual(findings1[0].ruleId, "CBS-001");
    assert.strictEqual(findings1[0].severity, "high");

    // CBS-002 Insecure Cipher Mode (ECB)
    const rule2 = rules.find((r) => r.id === "CBS-002");
    assert.ok(rule2, "CBS-002 rule should exist");
    const bomECB = makeBom([
      {
        type: "cryptographic-asset",
        name: "aes-ecb-cipher",
        cryptoProperties: {
    ...
    const findings2 = await evaluateRule(rule2, bomECB);
    assert.ok(
      findings2.length > 0,
      "Should detect ECB cipher mode as insecure",
    );
    assert.strictEqual(findings2[0].ruleId, "CBS-002");

    // CBS-003 Insufficient Security Level (1024-bit RSA key / 80 bits classical security)
    const rule3 = rules.find((r) => r.id === "CBS-003");
    assert.ok(rule3, "CBS-003 rule should exist");
    const bomWeakKey = makeBom([
      {
        type: "cryptographic-asset",
        name: "weak-key",
        cryptoProperties: {
    ...
    const findings3 = await evaluateRule(rule3, bomWeakKey);
    assert.ok(findings3.length > 0, "Should detect short RSA key size");
    assert.strictEqual(findings3[0].ruleId, "CBS-003");

    // CBS-004 Insecure Protocol Version (TLS 1.0)
    const rule4 = rules.find((r) => r.id === "CBS-004");
    assert.ok(rule4, "CBS-004 rule should exist");
    const bomTLS10 = makeBom([
      {
        type: "cryptographic-asset",
        name: "tls-conn",
        cryptoProperties: {
    ...
    const findings4 = await evaluateRule(rule4, bomTLS10);
    assert.ok(findings4.length > 0, "Should detect TLS 1.0 protocol version");
    assert.strictEqual(findings4[0].ruleId, "CBS-004");
    assert.strictEqual(findings4[0].severity, "critical");

    // CBC-001 Expired Certificate
    const rule5 = rules.find((r) => r.id === "CBC-001");
    assert.ok(rule5, "CBC-001 rule should exist");
    const bomExpiredCert = makeBom([
      {
        type: "cryptographic-asset",
        name: "expired-cert",
        properties: [{ name: "cdx:cert:isExpired", value: "true" }],
        cryptoProperties: {
    ...
    const findings5 = await evaluateRule(rule5, bomExpiredCert);
    assert.ok(findings5.length > 0, "Should detect expired certificate");
    assert.strictEqual(findings5[0].ruleId, "CBC-001");
  });
});
